// controllers/fleet-management/deliveryNoteSubSalesOrderController.js
...
// Used to populate the checkbox tables in the "Create DN for Another Sub SO" UI
...
const createSubSalesOrder = async (req, res) => {
  ...
  try {
    ...
    const subSO = await DeliveryNoteSubSalesOrderModel.create(
      {
        sales_order_id,
    ...
    res.status(201).json({
      success: true,
      message: "Sub Sales Order created successfully",
      subSalesOrder: subSO,
    });
  } catch (error) {
    await transaction.rollback();
    console.error("Error creating Sub Sales Order:", error);
    res.status(500).json({ error: error.message });
  }
};
...
const getSubSalesOrderById = async (req, res) => {
  try {
    ...
    const subSO = await DeliveryNoteSubSalesOrderModel.findByPk(sub_so_id, {
      include: [
        {
    ...
    if (!subSO) {
      return res.status(404).json({ message: "Sub Sales Order not found" });
    }

    res.status(200).json({ success: true, subSalesOrder: subSO });
  } catch (error) {
    console.error("Error fetching Sub Sales Order:", error);
    res.status(500).json({ error: error.message });
  }
};
...
const getAllSubSalesOrders = async (req, res) => {
  try {
    ...
    const subSOs = await DeliveryNoteSubSalesOrderModel.findAll({
      where: whereClause,
      include: [
        {
          model: SalesOrdersModel,
    ...
    console.error("Error fetching Sub Sales Orders:", error);
    res.status(500).json({ error: error.message });
  }
};
...
const updateSubSalesOrder = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Sub Sales Order updated successfully",
      subSalesOrder: await DeliveryNoteSubSalesOrderModel.findByPk(sub_so_id, {
        include: [{ model: SalesOrdersModel, as: "salesOrder" }],
      }),
    });
  } catch (error) {
    await transaction.rollback();
    console.error("Error updating Sub Sales Order:", error);
    res.status(500).json({ error: error.message });
  }
};
...
const deleteSubSalesOrder = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, message: "Sub Sales Order deleted successfully" });
  } catch (error) {
    await transaction.rollback();
    console.error("Error deleting Sub Sales Order:", error);
    res.status(500).json({ error: error.message });
  }
};

// ── 8. Get DN Trip Summary data for a Sub SO ────────────────────────────────
// Returns the selected resources in the format needed for the DN Trip Summary Table
const getSubSOTripSummary = async (req, res) => {
  try {
    ...
    console.error("Error fetching Sub SO trip summary:", error);
    res.status(500).json({ error: error.message });
  }
};
