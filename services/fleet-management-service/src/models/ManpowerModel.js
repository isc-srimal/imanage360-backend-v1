...
const OperatorTypeModel = require("./OperatorTypeModel");
...
const ManpowerModel = sequelize.define(
  "tbl_manpower",
  {
    manpower_id: {
      type: DataTypes.INTEGER,
    ...
    gatePassAttachment: {
      ...
      defaultValue: "",
      get() {
        const value = this.getDataValue("gatePassAttachment");
        return value
          ? `services/fleet-management-service/public/uploads/gatePassAttachmentDocuments/${value}`
          : "";
      },
      set(value) {
        // Store only the filename, not the full path
        ...
        if (value) {
          if (typeof value === "string") {
            ...
            filename = value
              .replace(/^.*[\\\/]/, "")
              .replace(/^\/services\/fleet-management-service\/public\/uploads\/gatePassAttachmentDocuments\//, "");
          } else {
            // If it's a File object or already a filename
            filename = value;
          }
        }
        this.setDataValue("gatePassAttachment", filename);
      },
    },
    ...
    manpower_status_note: {
      type: DataTypes.TEXT,
      allowNull: true,
      comment: "Note for statuses that require explanation (e.g., sick-leave)",
    },
    created_at: {
      type: DataTypes.DATE,
...
ManpowerModel.belongsTo(EmployeeModel, {
  foreignKey: "employeeId",
  as: "employee",
});
EmployeeModel.hasMany(ManpowerModel, {
  foreignKey: "employeeId",
  as: "manpower",
});

ManpowerModel.belongsTo(OperatorTypeModel, {
  foreignKey: "operator_type_id",
  as: "operator_type",
});
OperatorTypeModel.hasMany(ManpowerModel, {
  foreignKey: "operator_type_id",
  as: "manpower",
});

ManpowerModel.belongsTo(EquipmentModel, {
  foreignKey: "serial_number",
  as: "equipment",
});
EquipmentModel.hasMany(ManpowerModel, {
  foreignKey: "serial_number",
  as: "manpower",
});
